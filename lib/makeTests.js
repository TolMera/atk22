...
function makeTests(
    ...
    return () => {
        ...
        if (params.returns) testFnWithUserExpectedReturns(params, fn);

        if (params.advanced.tests) testFnWithParamsAdvancedTests(params, fn);
        ...
        if (params.inputs) automatedTestingWithGeneratorsFromParamsInputs(params, fn);
...
const testFnWithParamsAdvancedTests = (params, fn) => {
    for (let testset of params.advanced.tests) {
        ...
        for (let index of Object.keys(params.returns)) {
            const result = params.returns[index];
            for (let input of result) {
                const res = fn(...input);
                if (res.toString() !== index) {
...
const testFnWithUserExpectedReturns = (params, fn) => {
    for (let index of Object.keys(params.returns)) {
        ...
        for (let input of result) {
            const res = fn(...input);
            if (res.toString() !== index) {
                throw new Error(`Function returned an unexpected result, (${res.toString()} !== ${result})`);
            }
        }
    }
}
